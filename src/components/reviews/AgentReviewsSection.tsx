...
import { useAgentReviews, useAgentRating, useAgentReviewVotes, voteOnAgentReview, deleteAgentReview, AgentReviewSortOption } from '@/hooks/useAgentReviews';
import AgentReviewForm from '@/components/reviews/AgentReviewForm';
...
const AgentReviewsSection: React.FC<AgentReviewsSectionProps> = ({
  agentId,
  agentName,
  currentUserId,
  currentUserName,
  ...
  const [sortBy, setSortBy] = useState<AgentReviewSortOption>('newest');
  ...
  const handleVote = async (reviewId: string, voteType: 'helpful' | 'not_helpful') => {
    ...
    await voteOnAgentReview(reviewId, currentUserId, voteType);
    await refetch();
      ...
      {reviews.length > 1 && (
        <div className="flex items-center justify-between mb-4">
          ...
          <div className="relative">
            <select
              value={sortBy}
              onChange={(e) => setSortBy(e.target.value as AgentReviewSortOption)}
...
export default AgentReviewsSection;
